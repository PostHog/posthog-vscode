...
export function classifyFlagType(flag: FeatureFlag | undefined): FlagType {
    ...
    if (filters?.payloads && typeof filters.payloads === 'object') {
        ...
        if (Object.values(payloads).some(v => v !== null && v !== undefined)) { return 'remote_config'; }
    }
    return 'boolean';
}
...
export function extractVariants(flag: FeatureFlag): { key: string; rollout_percentage: number }[] {
    const filters = flag.filters as Record<string, unknown> | undefined;
    if (filters?.multivariate && typeof filters.multivariate === 'object') {
        const mv = filters.multivariate as { variants?: { key: string; rollout_percentage: number }[] };
        if (mv.variants && mv.variants.length > 0) { return mv.variants; }
    }
    return [];
}
...
export function extractConditionCount(flag: FeatureFlag): number {
    const filters = flag.filters as Record<string, unknown> | undefined;
    if (!filters?.groups || !Array.isArray(filters.groups)) { return 0; }
